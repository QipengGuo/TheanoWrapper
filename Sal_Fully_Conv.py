...
train_func = theano.function([img_in, img_tar], [cost, img_out], updates=sgd(cost+cost_sum_sal, model.weightsPack.getW_list(), lr=lr, iterations=iterations), allow_input_downcast=True)
...
for i in xrange(50):
    for j in xrange(1000):
        ...
        acc = NP.mean(((net_out>0.5)==(label>0.5))==(label>0.5))
    ...
    for j in xrange(len(bmnist_test.data_)/test_batch_size):
        ...
        test_acc.append(NP.mean(((net_out>0.5)==(label>0.5))==(label>0.5)))
