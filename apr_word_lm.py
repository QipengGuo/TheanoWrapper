...
word_seq = word_seq.dimshuffle(1, 0, 2)
sc, _ = theano.scan(_step, sequences=[word_seq], outputs_info=[starts, T.zeros(word_seq.shape[0], 200), T.zeros(word_seq.shape[0], 200), T.zeros(word_seq.shape[0], 200)])
word_out = sc[0].dimshuffle(1, 0, 2)

EPSI = 1e-6
cost = T.mean(NN.categorical_crossentropy(T.clip(word_out, EPSI, 1.0-EPSI), label_seq))
test_func = theano.fucntion([word_seq, label_seq, starts], [cost, word_out], allow_input_downcast=True)
...
for i in xrange(50)
    for j in xrange(1000):
        ...
        acc = 1.0 * NP.mean(NP.armgax(net_out, 2)==NP.argmax(label, 2))
    ...
    for j in xrange(amazon_data.test_size/test_batch_size):
        ...
        n_cost, net_out = test_fucn(data, label, NP.zeros((train_batch_size, len(data[0][0]))))
