	...
	def __init__(self, train_fname=None, test_fname=None, dict_fname=None, dataset_fnames=[], gen_dict=False, only_dict=True, DICT_SIZE=10000, raw_file=False):	
		    ...
		    self.c_dim = 1
                ...
                if gen_dict:
                    ...
                    self.reduce_dict(DICT_SIZE=DICT_SIZE)
        ...
        def get_batch(self, batch_size=32, test=False):
		...
		return char, char_label, mask, word_idx1, word_idx2, word_label
                ...
                for s in slist:
                    ...
                    for w in ws:
                        ...
                        if self.Wcnt[w] == 1:
                            ...
                            for c in w:
                                if not self.Cdict.has_key(c):
                                    self.Cdict[c]=self.c_dim
                                    self.num2c[self.c_dim]=c
                                    self.c_dim += 1
                f.close()

        def reduce_dict(self, DICT_SIZE=10000):
                Wcnt = OrderedDict((sorted(self.Wcnt.items(), key = lambda x:x[1], reverse=True)))
                items = Wcnt.items()
                for i in xrange(min(DICT_SIZE, len(Wcnt.keys()))):
                    self.Wdict[items[i][0]] = i+1 #start from 1, keep 0 to OOV
                    self.num2w[i+1]=items[i][0]
                self.w_dim = min(DICT_SIZE, len(Wcnt.keys()))+1
