...
def decode(enc, mask, pred_char, prev_h1, word_emb):
	...
	gru1 = batch_mask * model.gru(cur_in = T.concatenate((pred_char, word_emb), axis=1), rec_in = prev_h1, name = 'gru_dec', shape = (out_dim+mem_dim, gru2_dim))
...
enc = sc[-2][::-1]
rev_mask = cw_mask[:, ::-1]
sc,_ = theano.scan(decode, sequences=[enc, T.concatenate((T.ones((char_in.shape[0], 1)), rev_mask[:,:-1]), axis=1).dimshuffle([1, 0])], outputs_info = [T.zeros((char_in.shape[0], out_dim)), T.zeros((char_in.shape[0], gru2_dim)), T.zeros((char_in.shape[0], mem_dim))])

char_dec = (sc[0][::-1]).dimshuffle([1, 0, 2])
