...
def _gen_word_emb_step_ema(h_context_free, h_before):
	...
	hw_h = high_way(cur_in = D_h, name = 'hw_emb_h', shape=(emb_dim+gru1_dim, emb_dim+gru1_dim))
	word_emb = batch_mask * T.tanh(model.fc(cur_in = hw_h, name = 'fc_emb', shape = (emb_dim+gru1_dim, emb_dim)))
...
def _gen_word_emb_step_gru(h_context_free, h_before):
	...
	D_h = concatenate([D_h_c, D_h_cf], axis=1)
	hw_h = high_way(cur_in = D_h, name = 'hw_emb_h', shape=(emb_dim+gru1_dim, emb_dim+gru1_dim))
	word_emb = batch_mask * model.fc(cur_in = hw_h, name = 'fc_emb', shape = (emb_dim+gru1_dim, emb_dim))
...
def word_step(word_emb, prev_h1):
	...
	gru1 = model.gru(cur_in = word_emb, rec_in = prev_h1, name = 'gru_word', shape = (emb_dim, gru2_dim))
...
#        gru2 = model.gru(cur_in = D_gru1, rec_in = prev_h2, name = 'gru_word_2', shape = (gru2_dim, gru2_dim))

	return batch_mask * gru1 + (1-batch_mask) * prev_h1, gru1

def word_step2(h, prev_h1):
	batch_mask = get_mask(h)

	gru1 = model.gru(cur_in = h, rec_in = prev_h1, name = 'gru_word_2', shape = (gru2_dim, gru2_dim))
...
#                word_embs = h_context_free
	...
	word_out = log_softmax(model.fc(cur_in = word_out, name = 'fc_word', shape=(gru2_dim, word_out_dim)))
